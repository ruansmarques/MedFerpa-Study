...
// 5th Period Schedule Template (Horários Fixos com ID de Slot)
...
export const ScheduleView: React.FC<ScheduleViewProps> = ({ onNavigateToClass, initialDate }) => {
  ...
  const formatSubjectTitle = (title: string) => {
    if (title.includes(':')) {
        ...
        return <>{parts[0]}:<br/><span className="font-normal opacity-90">{parts[1].trim()}</span></>;
    }
    return title;
  };
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2">
           ...
           <div className="px-4 font-medium text-slate-700 w-32 text-center text-sm lg:text-base">{currentWeekStart.toLocaleDateString('pt-BR', { day: 'numeric', month: 'short' })}</div>
           ...
           <button onClick={() => setCurrentDate(new Date())} className="hidden sm:block px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-gray-50">Hoje</button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 w-full sm:grid sm:grid-cols-5 sm:divide-x sm:divide-gray-200 bg-gray-50/50 min-h-[500px]">
            {weekDays.map((day, dayIndex) => {
                ...
                return (
                    <div key={dayIndex} className="w-full relative p-2 space-y-2">
                        {isExamWeek ? (
                             ...
                             <div className="h-full flex items-center justify-center opacity-30"><p className="text-sm font-medium">-</p></div>
                        ) : (
                            daysEvents.map((event, idx) => {
                                const subject = getSubject(event.subjectId);
                                if (!subject) return null;

                                // --- NOVA LÓGICA DE FILTRAGEM POR SLOT ---
                                // Busca um documento que corresponda ao dia, disciplina E contenha este slot específico
                                ...
                                if (foundLesson && foundLesson.type === 'notice') {
                                    return (
                                        <div key={idx} className="w-full text-center rounded-xl h-48 overflow-hidden bg-white border-2 border-dashed border-blue-200 flex flex-col items-center justify-center p-4 gap-2 shadow-sm animate-fade-in">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full text-center rounded-xl shadow-sm hover:shadow-md hover:scale-[1.02] transition-all group flex flex-col h-48 overflow-hidden animate-fade-in ${getSubjectColor(event.subjectId)}`}
                                    >
                                        <div className="text-white w-full h-full flex flex-col">
                                            <div className="flex-1 flex flex-col items-center justify-center p-2 gap-1">
                                                <div className="text-[11px] sm:text-xs font-bold leading-tight tracking-wide">{formatSubjectTitle(subject.title)}</div>
                                                <p className="text-[10px] opacity-80 font-mono tracking-tighter">{event.startTime} – {event.endTime}</p>
                                            </div>
